...
export function Settings({ embedded = false }: { embedded?: boolean }) {
  ...
  const exportJSON = () =>
    download('adoption-adjudications.json', JSON.stringify(buildResultsJSON(dataset, results), null, 2))
  const exportCSV = () =>
    download('adoption-adjudications.csv', buildResultsCSV(items, results), 'text/csv')
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {!embedded && (
          <>
            <Section title="Queue">
              <Toggle label="Order" value={settings.queueMode} options={[['group', 'By backlog'], ['state', 'By state'], ['priority', 'Priority']]} onChange={(v) => setSettings({ queueMode: v as never })} />
        ...
        <Section title="Dataset">
          <p className="mb-2 text-xs text-slate-500">
            ...
            {dataset && <> · {dataset.meta.name} · {items.length} bundles · sources: {dataset.meta.sources.join(', ')}</>}
          </p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {source === 'opfs' && (
              <Btn danger onClick={() => { void (async () => { await clearOpfsDataset(); await reload() })() }}>Remove imported dataset</Btn>
            )}
          </div>
          {busy && <p className="mt-2 text-xs text-sky-300">{busy}</p>}
        </Section>
          ...
          <Section title="Danger zone">
            <Btn danger onClick={() => {
              if (confirm('Delete ALL your adjudications? This cannot be undone (export first).')) {
                void (async () => { await db.results.clear(); await reload() })()
              }
